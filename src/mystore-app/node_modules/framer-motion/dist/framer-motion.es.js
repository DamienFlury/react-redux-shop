import { useRef, useEffect, memo, forwardRef, createContext, useMemo, useContext, createElement, useCallback, useLayoutEffect, useState, Fragment, cloneElement, Children, isValidElement } from 'react';
import sync, { getFrameData, cancelSync } from 'framesync';
import { velocityPerSecond, distance, mix, interpolate, wrap } from '@popmotion/popcorn';
import styler, { createStylerFactory, buildStyleProperty, isTransformProp, buildSVGAttrs } from 'stylefire';
...
function useValueAnimationControls(config, props, inheritVariantChanges) {
    ...
    useEffect(function () { return function () { return parentControls && parentControls.removeChild(controls); }; }, []);
...
var createMotionComponent = function (_a) {
    ...
    function MotionComponent(props, externalRef) {
        ...
        var handleAnimate = Animate && (createElement(Animate, __assign({}, props, { inherit: shouldInheritVariant, innerRef: ref, values: values, controls: controls })));
        var handleActiveFunctionality = useFunctionalityComponents(props, values, controls, ref, style, context.static);
        return (createElement(MotionContext.Provider, { value: context },
...
function usePanGesture(_a, ref) {
    ...
    var updatePoint = useCallback(function () {
        if (!session.current ||
    ...
    var onPointerMove = useCallback(function (event, info) {
    ...
    var onPointerUp = useCallback(function (event, info) {
    ...
    var onPointerDown = useCallback(function (event, info) {
...
function useDraggable(_a, ref, values, controls) {
    ...
    var dragHandlers = useRef(handlers);
    ...
    var resolvedConstraints = useRef(false);
    ...
    var panHandlers = useMemo(function () {
        if (!drag)
            return {};
...
var buildSVGProps = function (values, style) {
    var motionValueStyles = resolveCurrent(values);
    var props = buildSVGAttrs(motionValueStyles);
...
function createDomMotionConfig(Component) {
    ...
    var RenderComponent = function (_a) {
        ...
        return createElement(Component, __assign({}, forwardProps, { ref: innerRef }, staticVisualStyles));
    };
    return {
        /**
        ...
        useFunctionalityComponents: function (props, values, controls, ref, style, isStatic) {
            ...
            if (!isStatic && Position.shouldRender(props)) {
                activeComponents.push(createElement(Position.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "position" })));
            }
            if (!isStatic && Drag.shouldRender(props)) {
                activeComponents.push(createElement(Drag.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "drag" })));
            }
            if (!isStatic && Gestures.shouldRender(props)) {
                activeComponents.push(createElement(Gestures.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "gestures" })));
            }
            activeComponents.push(createElement(RenderComponent, { componentProps: props, values: values, controls: controls, innerRef: ref, style: style, isStatic: isStatic, key: "renderComponent" }));
...
var AnimatePresence = function (_a) {
    ...
    var _c = useState(0), forcedRenderCount = _c[0], setForcedRenderCount = _c[1];
    ...
    if (isInitialRender.current) {
        ...
        return (createElement(Fragment, null, filteredChildren.map(function (child) {
            return cloneElement(child, { initial: false });
        })));
    }
    ...
    exiting.forEach(function (key) {
        ...
        childrenToRender.splice(insertionIndex, 0, cloneElement(child, __assign({}, props, { animate: exit || animate, onAnimationComplete: function () {
                exiting.delete(key);
...
export { AnimatePresence, AnimationControls, MotionContext, MotionPluginContext, MotionPlugins, MotionValue, Point, animationControls, createMotionComponent, motion, motionValue, safeWindow, transform, unwrapMotionValue, useAnimatedState, useAnimation, useCycle, useExternalRef, useGestures, useMotionValue, usePanGesture, useSpring, useTapGesture, useTransform, useViewportScroll };
