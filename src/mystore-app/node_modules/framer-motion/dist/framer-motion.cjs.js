...
var buildStyleAttr = function (values, styleProp, isStatic) {
    ...
    return __assign({}, styleProp, styler.buildStyleProperty(motionValueStyles, !isStatic));
};
...
function startAnimation(key, value, target, _a) {
    var _b = _a.delay, delay = _b === void 0 ? 0 : _b, transition = __rest(_a, ["delay"]);
...
var ValueAnimationControls = /** @class */ (function () {
    ...
    ValueAnimationControls.prototype.checkForNewValues = function (target) {
        ...
        for (var i = 0; i < numNewValues; i++) {
            ...
            else if (!getValueType(value) && styleValueTypes.complex.test(target[key])) {
                // If value is not recognised as animatable, ie "none", create an animatable version origin based on the target
                value = styleValueTypes.complex.getAnimatableNone(target[key]);
            }
...
function useValueAnimationControls(config, props, inheritVariantChanges) {
    ...
    React.useEffect(function () { return function () { return parentControls && parentControls.removeChild(controls); }; }, []);
...
var createMotionComponent = function (_a) {
    ...
    function MotionComponent(props, externalRef) {
        ...
        var handleAnimate = Animate && (React.createElement(Animate, __assign({}, props, { inherit: shouldInheritVariant, innerRef: ref, values: values, controls: controls })));
...
function usePanGesture(_a, ref) {
    ...
    var updatePoint = React.useCallback(function () {
        if (!session.current ||
            pointer.current === null ||
            lastMoveEventInfo.current === null ||
            lastMoveEvent.current === null) {
            heyListen.warning(false, "Pointer move without started session");
            stopPointerMove();
            stopPointerUp();
    ...
    var onPointerMove = React.useCallback(function (event, info) {
    ...
    var onPointerUp = React.useCallback(function (event, info) {
        sync.cancelSync.update(updatePoint);
        stopPointerMove();
        stopPointerUp();
        if (!session.current || pointer.current === null) {
            heyListen.warning(false, "Pointer end without started session");
            unblockViewportScroll();
            return;
        }
        if (onPanEnd) {
    ...
    var onPointerDown = React.useCallback(function (event, info) {
...
function useDraggable(_a, ref, values, controls) {
    ...
    var dragHandlers = React.useRef(handlers);
    ...
    var resolvedConstraints = React.useRef(false);
...
function usePositionAnimation(ref, values, controls, positionTransition) {
    ...
    React.useLayoutEffect(function () {
        ...
        if (prev && target && hasMoved(prev, target)) {
            ...
            styler__default(ref.current).render();
...
var Position = {
    shouldRender: function (props) { return !!props.positionTransition; },
    component: makeHookComponent(function (_a) {
        var innerRef = _a.innerRef, controls = _a.controls, values = _a.values, positionTransition = _a.positionTransition;
        return usePositionAnimation(innerRef, values, controls, positionTransition);
    }),
};
...
function stripMotionProps(_a) {
    var initial = _a.initial, animate = _a.animate, exit = _a.exit, style = _a.style, variants = _a.variants, transition = _a.transition, transformTemplate = _a.transformTemplate, custom = _a.custom, inherit = _a.inherit, s = _a.static, positionTransition = _a.positionTransition, onAnimationComplete = _a.onAnimationComplete, onUpdate = _a.onUpdate, onDragStart = _a.onDragStart, onDrag = _a.onDrag, onDragEnd = _a.onDragEnd, onDirectionLock = _a.onDirectionLock, onDragTransitionEnd = _a.onDragTransitionEnd, drag = _a.drag, dragConstraints = _a.dragConstraints, dragDirectionLock = _a.dragDirectionLock, dragElastic = _a.dragElastic, dragMomentum = _a.dragMomentum, dragPropagation = _a.dragPropagation, dragTransition = _a.dragTransition, dragOriginX = _a.dragOriginX, dragOriginY = _a.dragOriginY, onPan = _a.onPan, onPanStart = _a.onPanStart, onPanEnd = _a.onPanEnd, onPanSessionStart = _a.onPanSessionStart, onTap = _a.onTap, onTapStart = _a.onTapStart, onTapCancel = _a.onTapCancel, whileHover = _a.whileHover, whileTap = _a.whileTap, onHoverEnd = _a.onHoverEnd, onHoverStart = _a.onHoverStart, props = __rest(_a, ["initial", "animate", "exit", "style", "variants", "transition", "transformTemplate", "custom", "inherit", "static", "positionTransition", "onAnimationComplete", "onUpdate", "onDragStart", "onDrag", "onDragEnd", "onDirectionLock", "onDragTransitionEnd", "drag", "dragConstraints", "dragDirectionLock", "dragElastic", "dragMomentum", "dragPropagation", "dragTransition", "dragOriginX", "dragOriginY", "onPan", "onPanStart", "onPanEnd", "onPanSessionStart", "onTap", "onTapStart", "onTapCancel", "whileHover", "whileTap", "onHoverEnd", "onHoverStart"]);
    return props;
}
var buildSVGProps = function (values, style) {
    var motionValueStyles = resolveCurrent(values);
    var props = styler.buildSVGAttrs(motionValueStyles);
...
function createDomMotionConfig(Component) {
    ...
    var RenderComponent = function (_a) {
        ...
        return React.createElement(Component, __assign({}, forwardProps, { ref: innerRef }, staticVisualStyles));
    };
    return {
        /**
        ...
        useFunctionalityComponents: function (props, values, controls, ref, style, isStatic) {
            ...
            if (!isStatic && Position.shouldRender(props)) {
                activeComponents.push(React.createElement(Position.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "position" })));
            }
            if (!isStatic && Drag.shouldRender(props)) {
                activeComponents.push(React.createElement(Drag.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "drag" })));
            }
            if (!isStatic && Gestures.shouldRender(props)) {
                activeComponents.push(React.createElement(Gestures.component, __assign({}, props, { values: values, controls: controls, innerRef: ref, key: "gestures" })));
            }
            activeComponents.push(React.createElement(RenderComponent, { componentProps: props, values: values, controls: controls, innerRef: ref, style: style, isStatic: isStatic, key: "renderComponent" }));
...
function transform() {
    ...
    var interpolator = popcorn.interpolate(inputRange, outputRange, __assign({ mixer: getMixer(outputRange[0]) }, options));
...
function useSpring(source, config) {
    ...
    React.useMemo(function () {
        return value.attach(function (v, set) {
            ...
            activeSpringAnimation.current = popmotion.spring(__assign({ from: value.get(), to: v, velocity: value.getVelocity() }, config)).start(set);
            return value.get();
        });
...
var AnimatePresence = function (_a) {
    ...
    var _c = React.useState(0), forcedRenderCount = _c[0], setForcedRenderCount = _c[1];
    ...
    if (isInitialRender.current) {
        ...
        if (initial)
            return React.createElement(React.Fragment, null, filteredChildren);
        ...
        return (React.createElement(React.Fragment, null, filteredChildren.map(function (child) {
            return React.cloneElement(child, { initial: false });
        })));
    }
    ...
    exiting.forEach(function (key) {
        ...
        childrenToRender.splice(insertionIndex, 0, React.cloneElement(child, __assign({}, props, { animate: exit || animate, onAnimationComplete: function () {
                exiting.delete(key);
